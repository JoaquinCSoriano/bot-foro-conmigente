...
def run():
    ...
    with sync_playwright() as p:
        ...
        # Volvemos al contexto estable de los primeros pasos
        context = browser.new_context(user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
        page = context.new_page()

        # 1. LOGIN (El que ya tenías solucionado)
        ...
        cuerpo_email = "REPORTE DETALLADO DE BARRANCOS:\n\n"

        # 4. CAPTURA DE TEXTO MEJORADA (Esto es lo único que cambia respecto al #14)
        for url in urls:
            page.goto(url)
            page.wait_for_timeout(4000)
            titulo = page.title().split("-")[0].strip()
            
            # Buscamos en el contenedor general del foro para no fallar
            post = page.query_selector('#wpforo-wrap')
            texto = post.inner_text().strip() if post else "Texto no localizable."
