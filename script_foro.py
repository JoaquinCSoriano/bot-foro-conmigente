...
def run():
    ...
    with sync_playwright() as p:
        ...
        # Recuperamos el agente que el foro SÍ acepta
        context = browser.new_context(user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
        page = context.new_page()

        # 1. LOGIN ESTABLE (Copiado de la versión que daba verde)
        ...
        # 4. CAPTURA DE TEXTO (La mejora para Zapier)
        for url in urls:
            page.goto(url)
            page.wait_for_timeout(4000)
            titulo = page.title().split("-")[0].strip()
            
            # Usamos el contenedor general del foro para asegurar la lectura
